...
class define:
    ...
    class program_node(SelfNode):
        ...
        def compile(self, global_c: Context, local_c: Context) -> []:
            ...
            code_exec = types.CodeType(0, 0, 0, 0, 0, 0, code_bytes, tuple(co_consts), tuple(co_names), (),
                                       "", "", 0, b'', cellvars=tuple(co_varnames))
            return code_exec
    ...
    class func_node(SelfNode):
        ...
        def compile(self, global_c: Context, local_c: Context) -> []:
            ...
            co_names = lc.names
            code_exec = types.CodeType(len(lc.getExtra("args")), 0, 0, len(co_varnames), 0, 67, code_bytes, tuple(co_consts), tuple(co_names),
                                       tuple(co_varnames),
                                       "", idf, 0, b'', freevars=tuple(lc.getExtra("globals")), cellvars=())
            ...
            if lc.getExtra("globals"):
                return [f"LOAD_CLOSURE {global_c.getVar(name)[0]}" for name in lc.getExtra("globals")] + [
