...
class define:
    ...
    class program_node(SelfNode):
        ...
        def compile(self, global_c: Context, local_c: Context) -> []:
            ...
            code = list(map(exchange, release_code))

            co_consts = global_c.const_table
            co_globals = global_c.extra['co_globals']
            co_sym = ["" for _ in global_c.symbol_table]
            for k, (x, _) in global_c.symbol_table.items():
                co_sym[x] = k
            code_bytes = bytes.fromhex(Context.compile_byte_code(code))
            code_exec = types.CodeType(0, 0, 0, len(co_sym), 0, 0, code_bytes, tuple(co_consts), tuple(co_globals),
                                 tuple(co_sym),
                                 "", "", 0, b'')
            return code_exec
